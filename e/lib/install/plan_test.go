...
func (s *PlanSuite) SetUpSuite(c *check.C) {
	...
	c.Assert(err, check.IsNil)
	operationKey, err := s.services.Operator.CreateSiteInstallOperation(
...
func (s *PlanSuite) TestPlan(c *check.C) {
	op, err := s.services.Operator.GetSiteOperation(s.installer.OperationKey)
	c.Assert(err, check.IsNil)
	...
	expected := []struct {
		...
		{ephases.InstallerPhase, s.verifyInstallerPhase},
...
func (s *PlanSuite) verifyInstallerPhase(c *check.C, phase storage.OperationPhase) {
	storage.DeepComparePhases(c, storage.OperationPhase{
		ID: ephases.InstallerPhase,
		Data: &storage.OperationPhaseData{
			Package: &s.installer.AppPackage,
			Agent: &storage.LoginEntry{
				OpsCenterURL: s.installer.Config.RemoteOpsURL,
				Password:     s.installer.Config.RemoteOpsToken,
			},
		},
...
func (s *PlanSuite) verifyDecryptPhase(c *check.C, phase storage.OperationPhase) {
	storage.DeepComparePhases(c, storage.OperationPhase{
		...
		Requires: []string{ephases.InstallerPhase},
	}, phase)
}
...
func (s *PlanSuite) verifyConfigurePhase(c *check.C, phase storage.OperationPhase) {
	storage.DeepComparePhases(c, storage.OperationPhase{
		...
		Requires: []string{ephases.InstallerPhase, ephases.DecryptPhase},
		Data: &storage.OperationPhaseData{
...
func (s *PlanSuite) verifyLicensePhase(c *check.C, phase storage.OperationPhase) {
	storage.DeepComparePhases(c, storage.OperationPhase{
		...
		Data: &storage.OperationPhaseData{
			Server:  &s.masterNode,
			License: []byte(s.installer.Cluster.License.Raw),
		},
...
func (s *PlanSuite) verifyConnectPhase(c *check.C, phase storage.OperationPhase) {
	cluster, err := s.installer.makeTrustedCluster()
	c.Assert(err, check.IsNil)
