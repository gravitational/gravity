...
func (p *Process) maybeReconnectTrustedClusters(ctx context.Context) error {
	p.waitForOperator(ctx)
	local, err := p.operator.GetLocalSite()
	if err != nil {
		return trace.Wrap(err)
	}
	trustedClusters, err := p.operator.GetTrustedClusters(local.Key())
	if err != nil {
		return trace.Wrap(err)
	}
	for _, tc := range trustedClusters {
		...
		if needReconnect {
			...
			go p.reconnectTrustedCluster(ctx, local, tc)
		}
	}
	return nil
}
...
func (p *Process) reconnectTrustedCluster(ctx context.Context, local *ossops.Site, tc storage.TrustedCluster) {
	utils.RetryWithInterval(ctx, utils.NewUnlimitedExponentialBackOff(), func() error {
		...
		err := p.operator.DeleteTrustedCluster(ops.DeleteTrustedClusterRequest{
		...
		select {
		case <-timer.C:
			err = p.operator.UpsertTrustedCluster(local.Key(), tc)
			if err != nil {
				return trace.Wrap(err)
			}
		}
