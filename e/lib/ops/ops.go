...
type Endpoints interface {
	...
	UpdateClusterEndpoints(ops.SiteKey, storage.Endpoints) error
}
...
type PeriodicUpdates interface {
	...
	// Ops Center
	DownloadUpdate(DownloadUpdateRequest) error
}
...
type TrustedClusters interface {
	...
	UpsertTrustedCluster(ops.SiteKey, storage.TrustedCluster) error
	// DeleteTrustedCluster deletes a trusted cluster by name
	DeleteTrustedCluster(DeleteTrustedClusterRequest) error
...
type Identity interface {
	...
	UpsertRole(key ops.SiteKey, role services.Role) error
	// GetRole returns a role by name
	GetRole(key ops.SiteKey, name string) (services.Role, error)
	// GetRoles returns all roles
	GetRoles(key ops.SiteKey) ([]services.Role, error)
	// DeleteRole deletes a role by name
	DeleteRole(key ops.SiteKey, name string) error
	// UpsertOIDCConnector creates or updates an OIDC connector
	UpsertOIDCConnector(key ops.SiteKey, connector services.OIDCConnector) error
	...
	DeleteOIDCConnector(key ops.SiteKey, name string) error
	// UpsertSAMLConnector creates or updates a SAML connector
	UpsertSAMLConnector(key ops.SiteKey, connector services.SAMLConnector) error
	...
	DeleteSAMLConnector(key ops.SiteKey, name string) error
}
