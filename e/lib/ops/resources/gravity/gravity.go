...
func (r *Resources) Create(ctx context.Context, req resources.CreateRequest) error {
	...
	case storage.KindEndpoints:
		...
		if !cluster.Key().IsEqualTo(req.SiteKey) {
		...
		err = r.Operator.UpdateClusterEndpoints(ctx, cluster.Key(), endpoints)
		if err != nil {
			return trace.Wrap(err)
		}
	...
	case storage.KindEndpoints:
		...
		client, _, err := httplib.GetClusterKubeClient(cluster.DNSConfig.Addr())
		if err != nil {
