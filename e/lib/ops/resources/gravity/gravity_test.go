...
func (s *GravityResourcesSuite) TestRole(c *check.C) {
	err := s.r.Create(resources.CreateRequest{Resource: toUnknown(c, role)})
	c.Assert(err, check.IsNil)

	collection, err := s.r.GetCollection(resources.ListRequest{Kind: teleservices.KindRole, Name: role.GetName()})
	c.Assert(err, check.IsNil)
	err = role.CheckAndSetDefaults()
	c.Assert(err, check.IsNil)
	compare.DeepCompare(c, collection, &roleCollection{[]teleservices.Role{role}})

	err = s.r.Remove(resources.RemoveRequest{Kind: teleservices.KindRole, Name: role.GetName()})
	c.Assert(err, check.IsNil)
...
func (s *GravityResourcesSuite) TestOIDCConnectorResource(c *check.C) {
	err := s.r.Create(resources.CreateRequest{Resource: toUnknown(c, oidcConnector)})
	c.Assert(err, check.IsNil)

	collection, err := s.r.GetCollection(resources.ListRequest{Kind: teleservices.KindOIDCConnector, WithSecrets: true})
	c.Assert(err, check.IsNil)
	compare.DeepCompare(c, collection, &oidcCollection{[]teleservices.OIDCConnector{oidcConnector}})

	err = s.r.Remove(resources.RemoveRequest{Kind: teleservices.KindOIDCConnector, Name: oidcConnector.GetName()})
	c.Assert(err, check.IsNil)

	collection, err = s.r.GetCollection(resources.ListRequest{Kind: teleservices.KindOIDCConnector})
	c.Assert(err, check.IsNil)
...
func (s *GravityResourcesSuite) TestSAMLConnectorResource(c *check.C) {
	err := s.r.Create(resources.CreateRequest{Resource: toUnknown(c, samlConnector)})
	c.Assert(err, check.IsNil)

	collection, err := s.r.GetCollection(resources.ListRequest{Kind: teleservices.KindSAMLConnector, WithSecrets: true})
	c.Assert(err, check.IsNil)
	compare.DeepCompare(c, collection, &samlCollection{[]teleservices.SAMLConnector{samlConnector}})

	err = s.r.Remove(resources.RemoveRequest{Kind: teleservices.KindSAMLConnector, Name: samlConnector.GetName()})
	c.Assert(err, check.IsNil)

	collection, err = s.r.GetCollection(resources.ListRequest{Kind: teleservices.KindSAMLConnector})
	c.Assert(err, check.IsNil)
...
func (s *GravityResourcesSuite) TestAuthConnectorResource(c *check.C) {
	...
	collection, err := s.r.GetCollection(resources.ListRequest{Kind: teleservices.KindAuthConnector, WithSecrets: true})
	c.Assert(err, check.IsNil)
	...
	err = s.r.Remove(resources.RemoveRequest{Kind: teleservices.KindGithubConnector, Name: githubConnector.GetName()})
	c.Assert(err, check.IsNil)
