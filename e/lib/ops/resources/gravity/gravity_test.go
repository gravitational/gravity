...
func (s *GravityResourcesSuite) SetUpSuite(c *check.C) {
	...
	s.server = httptest.NewTLSServer(handler.NewWebHandler(s.s.Handler,
...
func (s *GravityResourcesSuite) TestRole(c *check.C) {
	...
	_, err = s.r.GetCollection(resources.ListRequest{SiteKey: s.cluster.Key(), Kind: teleservices.KindRole, Name: role.GetName()})
