...
func (o *Operator) UpsertTrustedCluster(key ossops.SiteKey, cluster storage.TrustedCluster) error {
	o.Infof("UpsertTrustedCluster(%s).", cluster)
	if !o.isInstaller() {
		local, err := o.GetLocalSite()
		if err != nil {
			return trace.Wrap(err)
		}
		if cluster.GetName() == local.Domain {
			return trace.BadParameter("can't connect to Ops Center with the "+
...
func (o *Operator) configureAccess(key ossops.SiteKey, cluster storage.TrustedCluster) error {
...
func (o *Operator) configureUpdates(key ossops.SiteKey, cluster storage.TrustedCluster) (err error) {
	if cluster.GetPullUpdates() {
		err = o.EnablePeriodicUpdates(ops.EnablePeriodicUpdatesRequest{
			AccountID:  key.AccountID,
			SiteDomain: key.SiteDomain,
		})
	} else {
		err = o.DisablePeriodicUpdates(key)
	}
	if err != nil {
		return trace.Wrap(err)
	}
...
func (o *Operator) DeleteTrustedCluster(req ops.DeleteTrustedClusterRequest) error {
	o.Infof("%s.", req)
	err := req.Check()
	if err != nil {
		return trace.Wrap(err)
	...
	if !cluster.GetWizard() && !cluster.GetSystem() {
		err = o.DisablePeriodicUpdates(req.SiteKey())
		if err != nil && !trace.IsNotFound(err) {
			return trace.Wrap(err)
		}
	}
...
func (o *Operator) upsertRemoteCluster(key ossops.SiteKey, cluster storage.TrustedCluster) error {
	...
	if trace.IsNotFound(err) {
		...
		o.Debugf("Created Ops Center agent %v.", agent.GetName())
	}
	caPackage, err := opsservice.PlanetCertAuthorityPackage(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
