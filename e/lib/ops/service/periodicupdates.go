...
func (o *Operator) DownloadUpdate(req ops.DownloadUpdateRequest) error {
	...
	err = puller.PullApp(context.TODO(), req.Application)
	if err != nil {
		return trace.Wrap(err)
	}
	...
	err = syncer.SyncApp(context.TODO(), req.Application)
	return trace.Wrap(err)
}

// EnablePeriodicUpdates turns periodic updates for the cluster on or updates the interval
func (o *Operator) EnablePeriodicUpdates(req ops.EnablePeriodicUpdatesRequest) error {
...
func (o *Operator) DisablePeriodicUpdates(key ossops.SiteKey) error {
	...
	for {
		...
		case <-ticker.C:
			...
			if update == nil {
				...
				err = o.DownloadUpdate(ops.DownloadUpdateRequest{
