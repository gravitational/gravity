...
func (o *Operator) NewLicense(req ops.NewLicenseRequest) (string, error) {
	if !o.isOpsCenter() {
...
func (o *Operator) CheckSiteLicense(key ossops.SiteKey) error {
...
func (o *Operator) UpdateLicense(req ops.UpdateLicenseRequest) error {
	...
	_, _, err = app.RunAppHook(context.TODO(), o.apps(), app.HookRunRequest{
...
func (o *Operator) GetLicenseCA() ([]byte, error) {
	if !o.isOpsCenter() {
		return nil, trace.BadParameter("not an Ops Center")
	}
	ca, err := pack.ReadCertificateAuthority(o.packages())
	if err != nil {
...
func InstallLicenseConfigMap(client *kubernetes.Clientset, licenseData string) error {
	...
	_, err := client.Core().ConfigMaps(defaults.KubeSystemNamespace).Create(configMap)
	if err == nil {
		return nil
	}
	if !trace.IsAlreadyExists(rigging.ConvertError(err)) {
...
func GetLicenseFromSecret(client *kubernetes.Clientset) (string, error) {
	secret, err := client.Core().Secrets(defaults.KubeSystemNamespace).Get(
...
func DeleteLicenseSecret(client *kubernetes.Clientset) error {
	err := rigging.ConvertError(client.Core().Secrets(defaults.KubeSystemNamespace).Delete(
...
func DeleteLicenseConfigMap(client *kubernetes.Clientset) error {
	err := rigging.ConvertError(client.Core().ConfigMaps(defaults.KubeSystemNamespace).Delete(
