...
func (r *Router) UpdateClusterEndpoints(key ossops.SiteKey, endpoints storage.Endpoints) error {
	client, err := r.remoteClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.UpdateClusterEndpoints(key, endpoints)
}
...
// Ops Center
func (r *Router) DownloadUpdate(req ops.DownloadUpdateRequest) error {
	client, err := r.pickClient(req.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.DownloadUpdate(req)
}

// EnablePeriodicUpdates turns periodic updates for the cluster on or
// updates the interval
func (r *Router) EnablePeriodicUpdates(req ops.EnablePeriodicUpdatesRequest) error {
	client, err := r.pickClient(req.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.EnablePeriodicUpdates(req)
}

// DisablePeriodicUpdates turns periodic updates for the cluster off and
// stops the update fetch loop if it's running
func (r *Router) DisablePeriodicUpdates(key ossops.SiteKey) error {
	client, err := r.pickClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.DisablePeriodicUpdates(key)
}
...
func (r *Router) UpsertTrustedCluster(key ossops.SiteKey, cluster storage.TrustedCluster) error {
	return r.Local.UpsertTrustedCluster(key, cluster)
}

// DeleteTrustedCluster deletes a trusted cluster by name
func (r *Router) DeleteTrustedCluster(req ops.DeleteTrustedClusterRequest) error {
	client, err := r.pickClient(req.ClusterName)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.DeleteTrustedCluster(req)
}
...
func (r *Router) NewLicense(req ops.NewLicenseRequest) (string, error) {
	return r.Local.NewLicense(req)
}

// CheckSiteLicense makes sure the license installed on cluster is correct
func (r *Router) CheckSiteLicense(key ossops.SiteKey) error {
	client, err := r.remoteClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.CheckSiteLicense(key)
}

// UpdateLicense updates license installed on cluster and runs a respective app hook
func (r *Router) UpdateLicense(req ops.UpdateLicenseRequest) error {
	client, err := r.remoteClient(req.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.UpdateLicense(req)
}
...
func (r *Router) UpsertRole(key ossops.SiteKey, role teleservices.Role) error {
	client, err := r.pickClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.UpsertRole(key, role)
}
...
func (r *Router) DeleteRole(key ossops.SiteKey, name string) error {
	client, err := r.pickClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.DeleteRole(key, name)
}

// UpsertOIDCConnector creates or updates an OIDC connector
func (r *Router) UpsertOIDCConnector(key ossops.SiteKey, connector teleservices.OIDCConnector) error {
	client, err := r.pickClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.UpsertOIDCConnector(key, connector)
}
...
func (r *Router) DeleteOIDCConnector(key ossops.SiteKey, name string) error {
	client, err := r.pickClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.DeleteOIDCConnector(key, name)
}

// UpsertSAMLConnector creates or updates a SAML connector
func (r *Router) UpsertSAMLConnector(key ossops.SiteKey, connector teleservices.SAMLConnector) error {
	client, err := r.pickClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.UpsertSAMLConnector(key, connector)
}
...
func (r *Router) DeleteSAMLConnector(key ossops.SiteKey, name string) error {
	client, err := r.pickClient(key.SiteDomain)
	if err != nil {
		return trace.Wrap(err)
	}
	return client.DeleteSAMLConnector(key, name)
}
