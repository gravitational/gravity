...
// of the remote site
func (c *stateChecker) syncSite(localSite storage.Site) error {
	remoteSite, err := c.conf.Operator.GetSite(ops.SiteKey{
		AccountID:  localSite.AccountID,
		SiteDomain: localSite.Domain,
	})
	if err != nil {
		return trace.Wrap(err)
	}
	...
	remoteState := remoteSite.State
	if localSite.State != remoteState {
		c.Infof("Cluster %v became %v (was: %v).", localSite.Domain, remoteState, localSite.State)

		changed = true
		localSite.State = remoteState
	}

	remotePackage := remoteSite.App.Package
	if !localSite.App.Locator().IsEqualTo(remotePackage) {
		c.Infof("Cluster %v app changed to %v (was: %v).", localSite.Domain, remotePackage, localSite.App.Locator())

		localEnvelope, err := c.conf.Packages.ReadPackageEnvelope(remotePackage)
		if err != nil {
			return trace.Wrap(err)
		}

		changed = true
		localSite.App = localEnvelope.ToPackage()
	}
