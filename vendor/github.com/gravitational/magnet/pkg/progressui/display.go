...
func (t *trace) calculateIndent(s *Vertex) string {
	if len(s.Inputs) != 0 {
		if parent, ok := t.byDigest[s.Inputs[0]]; ok {
			return fmt.Sprint(strings.Repeat(" ", len(parent.indent)), "- ")
		}
...
func (t *trace) sortedVertexes() []*vertex {
	//res := make([]*vertex, len(t.vertexes))
	var res []*vertex

	for _, v := range t.vertexes {
		if len(v.Inputs) == 0 {
			res = append(res, v)
			res = append(res, t.collectVertexesByParent(v.Digest)...)
		}
	}

	// collect any orphanrd vertexes, that have an input of a vertex that might be missing
	for _, v := range t.vertexes {
		var found bool

		for _, r := range res {
			if r.Digest == v.Digest {
				found = true
			}
		}
...
func (t *trace) collectVertexesByParent(parent digest.Digest) []*vertex {
	//res := make([]*vertex, len(t.vertexes))
	var res []*vertex

	for _, v := range t.vertexes {
		if len(v.Inputs) != 0 {
			if v.Inputs[0] == parent {
				res = append(res, v)
				res = append(res, t.collectVertexesByParent(v.Digest)...)
			}
		}
