...
const ReinviteDialog = React.createClass({

  render() {
    let { userId, attemp, onContinue, onCancel} = this.props;

    if( !userId ){
      return null;
    }

    let { isProcessing, isFailed, isSuccess, message } = attemp;

    if ( isSuccess ){
      return (
         <UserTokenLink tokenType="invite" userToken={message} onClose={onCancel}/>
      )
    }
      ...
      <GrvDialog title="" className="grv-dialog-no-body grv-dialog-sm grv-dialog-confirm grv-settings-dialog-with-errors">
        <GrvDialogHeader>
          <div className="grv-dialog-confirm-header">
            ...
            <div>
              <h3 className="m-b-xs">Renew the user's invitation</h3>
              <div>
                <small>
                  You are about to renew the user's invitation.
