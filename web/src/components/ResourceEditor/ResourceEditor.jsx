...
import Dialog, { DialogHeader, DialogTitle } from 'shared/components/Dialog';
...
export default function ResourceEditor(props) {
  ...
  const onSave = () => {
    actions.do(() => props.onSave(resource))
      .then(() => onClose());
  };

  const isSaveDisabled = attempt.isProcessing || ( !resource.isDirty && !resource.isNew);
    ...
    <Dialog open={true} dialogCss={dialogCss} onClose={onClose}>
      <Flex flex="1" m={-5} >
        <Flex flex="1" height="100%" p="5" flexDirection="column" >
          ...
          { !isNew && <Text mb="2" typography="h4" color="primary.contrastText">{name}</Text>}
