...
func (s *DockerSuite) SetUpTest(c *C) {
	...
	c.Assert(err, IsNil)
	s.helper = newDockerHelper(logrus.New(), s.client, utils.DiscardProgress)
...
func (s *DockerSuite) newRegistry(c *C) *registry {
	dir := c.MkDir()
...
func (r *registry) push(image loc.DockerImage, c *C) {
