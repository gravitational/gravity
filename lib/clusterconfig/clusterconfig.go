...
func (r *Updater) executePlan(ctx context.Context, machine *libfsm.FSM, force bool) error {
	progress := utils.NewProgress(ctx, "Updating cluster configuration", -1, false)
	defer progress.Stop()
...
func (r *Config) checkAndSetDefaults() error {
	...
	if r.FieldLogger == nil {
		r.FieldLogger = log.WithField(trace.Component, "cluster-config:updater")
	}
	return nil
}
...
// Updater executes a controlled update of cluster configuration on master nodes
type Updater struct {
