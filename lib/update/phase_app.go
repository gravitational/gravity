...
func (p *updatePhaseApp) Execute(ctx context.Context) error {
	...
	if p.Package.Name == constants.BootstrapConfigPackage {
		...
		err = p.runHooks(ctx, schema.HookOverlayUpdate, schema.HookUpdate, schema.HookUpdated)
	}
	if err != nil {
...
func (p *updatePhaseApp) Rollback(ctx context.Context) error {
	err := p.runHooks(ctx, schema.HookRollback, schema.HookRolledBack, schema.HookOverlayRollback)
	if err != nil {
		return trace.Wrap(err)
	}
	return nil
