...
func (s *site) getReport(ctx context.Context, runner remoteRunner, servers []remoteServer, master remoteServer) (io.ReadCloser, error) {
	...
	if len(servers) > 0 {
		...
		if err != nil {
			log.WithError(err).Warn("Failed to collect diagnostics from some nodes.")
		}
	}
...
func runCollectors(ctx context.Context, cluster site, dir string) error {
	...
	for _, collector := range collectors {
		...
		if err != nil {
			log.WithError(err).Warn("Failed to collect diagnostics.")
		}
	}
	return nil
}
...
func collectSiteInfo(s storage.Site) collectorFn {
	return func(_ context.Context, reportWriter report.FileWriter, cluster site) error {
