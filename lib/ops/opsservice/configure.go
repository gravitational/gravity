...
func (s *site) configureTeleportNode(ctx *operationContext, masterIP string, node *ProvisionedServer) error {
	...
	resp, err := s.getTeleportNodeConfig(ctx, masterIP, *configPackage, node)
	if err != nil {
		return trace.Wrap(err)
	}
	_, err = s.packages().CreatePackage(resp.Locator, resp.Reader, pack.WithLabels(resp.Labels))
...
func (s *site) getTeleportNodeConfig(ctx *operationContext, masterIP string, configPackage loc.Locator, node *ProvisionedServer) (*ops.RotatePackageResponse, error) {
	fileConf := &telecfg.FileConfig{
		Global: telecfg.Global{
			Ciphers:       defaults.TeleportCiphers,
			KEXAlgorithms: defaults.TeleportKEXAlgorithms,
