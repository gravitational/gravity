...
func (s *ChecksSuite) SetUpSuite(c *check.C) {
	...
	var err error
	s.checker, err = New(Config{})
	c.Assert(err, check.IsNil)
}

func (s *ChecksSuite) TestCheckCPU(c *check.C) {
	enoughCPU := schema.CPU{Min: 4}
	c.Assert(s.checker.checkCPU(s.info, enoughCPU), check.IsNil)

	notEnoughCPU := schema.CPU{Min: 5}
	c.Assert(s.checker.checkCPU(s.info, notEnoughCPU), check.NotNil)
}
...
func (s *ChecksSuite) TestCheckRAM(c *check.C) {
	...
	c.Assert(s.checker.checkRAM(s.info, notEnoughRAM), check.NotNil)
}
...
func (s *ChecksSuite) TestTime(c *check.C) {
	...
	c.Assert(s.checker.checkTime(now, nil), check.IsNil)
	...
	err := s.checker.checkTime(now, []Server{
