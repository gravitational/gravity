...
`gravity upgrade` will not uninstall existing applications that have been disabled in the upgrade manifest.
Example:
    ...
    The existing artifacts of the monitoring app will not be cleaned up. 
    The docker images will not be removed from the internal repository.
...
Gravity has three modes of label reconciliation, which the user can change at any time:
 - `EnsureExists` (default) - Compares the labels in the node with what is written in the profile, the value of the labels is not compared.
