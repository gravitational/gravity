...
func RegisterCommands(app *kingpin.Application) Application {
	...
	tele.BuildCmd.Overwrite = tele.BuildCmd.Flag("overwrite", "Overwrite the existing image file if it already exists.").Short('f').Bool()
	...
	tele.ListCmd.CmdClause = app.Command("ls", "Display a list of cluster and application images published to Gravity Hub.")
	...
	tele.PullCmd.Force = tele.PullCmd.Flag("force", "Overwrite the existing image file if it already exists.").Short('f').Bool()
