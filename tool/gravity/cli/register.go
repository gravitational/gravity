...
func RegisterCommands(app *kingpin.Application) *Application {
	...
	g.InstallCmd.DNSListenAddrs = g.InstallCmd.Flag("dns-listen-addr", "Custom listen address for planet dns.").Default(defaults.DNSListenAddr).IPList()
	g.InstallCmd.DNSPort = g.InstallCmd.Flag("dns-port", "Custom DNS port for planet dns.").Default(strconv.Itoa(defaults.DNSPort)).Int()
	...
	g.AppUninstallCmd.OpsCenterURL = g.AppUninstallCmd.Flag("ops-url", "optional remote OpsCenter URL").String()
