...
declare -A UPGRADE_NIGHTLY_MAP
# gravity version -> list of OS releases to exercise on
UPGRADE_MAP[5.0.24]="ubuntu:16"
UPGRADE_NIGHTLY_MAP[5.0.24]="redhat:7 centos:7 debian:9 suse:12 ubuntu:16"
...
function build_nightly_resize_suite {
  cat <<EOF
 resize={"to":3,"flavor":"one","nodes":1,"role":"node","state_dir":"/var/lib/telekube","os":"ubuntu:latest","storage_driver":"devicemapper"}
 resize={"to":6,"flavor":"three","nodes":3,"role":"node","state_dir":"/var/lib/telekube","os":"ubuntu:latest","storage_driver":"overlay2"}
EOF
}
...
function build_nightly_upgrade_suite {
  local suite=''
  local cluster_sizes=( \
    '"flavor":"three","nodes":3,"role":"node"' \
    '"flavor":"six","nodes":6,"role":"node"' \
    '"flavor":"one","nodes":1,"role":"node"')
  for release in ${!UPGRADE_NIGHTLY_MAP[@]}; do
    for os in ${UPGRADE_NIGHTLY_MAP[$release]}; do
...
function build_nightly_install_suite {
  local suite=''
